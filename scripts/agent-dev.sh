...
# "vikunja-mcp-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "vikunja-mcp-dev" is overwritten by
...
PROJECT_NAME="vikunja-mcp"
AGENT_NAME="vikunja-mcp-dev"
WORKSPACE="/workspaces/vikunja-mcp"
...
if [ -x "/workspaces/vikunja-mcp/scripts/agent-dev.sh" ]; then
  "/workspaces/vikunja-mcp/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
